...
static void find_common_ancestors(const NetPoint* src, const NetPoint* dst,
                                  ...
                                  std::vector<NetZoneImpl*>* path_dst)
{
  // xbt_enforce(src, "src is nullptr");
  // xbt_enforce(dst, "dst is nullptr");
  // xbt_enforce(src->get_englobing_zone(), "'%s'->get_englobing_zone() is nullptr", src->get_cname());
  // xbt_enforce(dst->get_englobing_zone(), "'%s'->get_englobing_zone() is nullptr", dst->get_cname());
  ...
  *common_ancestor    = nullptr;
  ...
  for (auto src_it = path_src->begin(), dst_it = path_dst->begin();
    ...
    XBT_DEBUG("src_it: %s, dst_it: %s", (*src_it)->get_cname(), (*dst_it)->get_cname());
    if (*src_it != *dst_it) /* (2) we found the difference at least. Finalize the returned values */
    {
      XBT_DEBUG("found the difference: src_it: %s, dst_it: %s", (*src_it)->get_cname(), (*dst_it)->get_cname());
...
/** @brief Get the path leading to the given netpoint from the root zone */
...
void NetZoneImpl::get_up_to_down_route(const NetPoint* down, NetPoint* up,
                                       ...
                                       std::vector<NetZoneImpl*>* path_src)
{
  XBT_DEBUG("get_up_to_down_route from '%s' to '%s' in '%s''", up->get_cname(),down->get_cname(), down->get_englobing_zone()->get_cname());
  ...
  xbt_enforce(down->get_englobing_zone() == gateway->get_englobing_zone(),
              "The final gateway '%s' and src '%s' are not in the same netzone, please report that bug.", gateway->get_cname(),
              down->get_cname());
                                                 ...
                                                 double* latency, std::unordered_set<NetZoneImpl*>& netzones)
{
  ...
  if (src_ancestor != common_ancestor && dst_ancestor != common_ancestor) {
    ...
    if (route.gw_src_ != nullptr)
      get_up_to_down_route(src, route.gw_src_, src_to_src_ancestor, latency, &path_src);
    else
    ...
    std::reverse(begin(src_to_src_ancestor), end(src_to_src_ancestor));
